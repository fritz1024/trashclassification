...
const initCharts = () => {
  ...
  if (trendChart.value) {
    ...
    chart2.setOption({
      ...
      series: [{
        type: 'line', data: stats.value.daily_stats.map(item => item.count), smooth: true, symbol: 'circle', symbolSize: 8,
...
onUnmounted(() => { chart1?.dispose(); chart2?.dispose(); window.removeEventListener('resize', handleResize) })
watch(() => route.path, (p) => { if (p === '/user/stats') fetchStats() })
...
.stat-card { display: flex; align-items: center; gap: var(--space-5); background: var(--bg-elevated); border: 1px solid var(--border-secondary); border-radius: var(--radius-xl); padding: var(--space-6); box-shadow: var(--shadow-sm); transition: all var(--transition-normal); }
...
.stat-value small { font-size: var(--text-lg); font-weight: var(--font-medium); }
.stat-label { font-size: var(--text-sm); color: var(--text-tertiary); margin-top: var(--space-1); }
.stat-info { display: flex; flex-direction: column; }
.charts-row { display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--space-5); }
.chart-card { background: var(--bg-elevated); border: 1px solid var(--border-secondary); border-radius: var(--radius-xl); padding: var(--space-6); box-shadow: var(--shadow-sm); }
.chart-card h3 { font-size: var(--text-lg); font-weight: var(--font-semibold); color: var(--text-primary); margin-bottom: var(--space-4); }
