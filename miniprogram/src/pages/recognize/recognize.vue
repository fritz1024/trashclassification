<template>
  <view class="container">
    ...
    <view class="mode-switch">
      <view class="mode-item" :class="{ active: mode === 'single' }" @click="switchMode('single')">
        单张识别
      </view>
      <view class="mode-item" :class="{ active: mode === 'batch' }" @click="switchMode('batch')">
        批量识别
      </view>
    </view>
    ...
    <view v-if="mode === 'single'" class="single-mode">
      <view class="upload-area" @click="chooseImage">
        <image v-if="imageUrl" :src="imageUrl" class="preview-image" mode="aspectFit" />
      ...
      <button class="recognize-btn" @click="recognizeSingle" :disabled="!imageUrl">
        开始识别
      </button>
      ...
      <view v-if="result" class="result-card">
        ...
        <view v-if="result.top3_results.length > 1" class="other-results">
          ...
          <view v-for="(item, index) in result.top3_results.slice(1)" :key="index" class="result-item">
    ...
    <view v-else class="batch-mode">
      <view class="batch-images">
        <view v-for="(img, index) in batchImages" :key="index" class="batch-image-item">
          <image :src="img" class="batch-image" mode="aspectFill" />
          <view class="delete-btn" @click="removeBatchImage(index)">×</view>
        </view>
        <view v-if="batchImages.length < 9" class="add-image-btn" @click="chooseBatchImages">
          <text class="add-icon">+</text>
          <text class="add-text">添加图片</text>
        </view>
      </view>

      <button class="recognize-btn" @click="recognizeBatch" :disabled="batchImages.length === 0">
        批量识别 ({{ batchImages.length }}/9)
      </button>
      ...
      <view v-if="batchResults.length > 0" class="batch-results">
        ...
        <view v-for="(item, index) in batchResults" :key="index" class="batch-result-item">
  ...
  methods: {
    ...
    async recognizeSingle() {
      ...
      try {
        ...
        uni.showToast({ title: '识别失败，请检查网络', icon: 'none' })
        this.result = null
      } finally {
        this.loading = false
      }
